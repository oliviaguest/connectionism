...
\lstdefinestyle{highlight}{  basicstyle=\ttfamily\color{black},
...
%   language=Python,
  ...
  breakatwhitespace=false, showspaces=false,showstringspaces=false, showtabs=false, breakatwhitespace=false, breaklines=false, 
  keywordstyle=\ttfamily\color{codebg},
...
Python output:
\begin{lstlisting}[style=base]
@2Please enter 'hello':no@
@3Please enter 'hello':never@
@4Please enter 'hello':hi?@
@5Please enter 'hello':hello@
@6>>>@
...
%  \vfill
\begin{itemize}
 \item  \href{www.codecademy.com}{\textbf{codecademy}.com} {\small (at least the first 8 lessons)}
 \item \href{www.learnpythonthehardway.org/book}{\textbf{learnpythonthehardway}.org/book} {\small (main textbook)}
 \item \textbf{StackExchange}: paste your error into Google
\end{itemize}
\vfill
Optional:
\begin{itemize}
  \item \href{http://www.learnpython.org/}{\textbf{learnpython}.org/}  {\small (alternative excercises)}
    \item \href{http://openbookproject.net/thinkcs/python/english2e/}{\textbf{openbookproject}.net/thinkcs/python/english2e} {\small (alternative textbook)}

  \item  \href{http://www.codewars.com}{\textbf{codewars}.com} {\small (programming puzzles)}
...
\framesubtitle{Important stuff}
\begin{enumerate}
  \item{What is \textbf{assignment}? \visible<2> {e.g., {\ttfamily   a == 6   } implies a is now 6 }}
  
  \item<3-> What \textbf{types of data} have we used so far? \visible<4> {e.g., Boolean}

  \item<5->  Why and when do we \textbf{indent}? \visible<6>{e.g., conditional statements}
  
  \item<7-> What types of flow control have we used so far?  \visible<8>{e.g., if}
  
  \item<9-> When are conditional statements used? 
  
  \item<11-> When are for-loops used?
  
  \item<13-> When are while-loops used?
